...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-slate-50 to-purple-50">
    ...
    <div class="bg-gradient-to-r from-[#1D293D] to-gray-900 shadow-2xl">
        <div class="px-4 py-4">
            ...
            <div class="flex items-center text-sm text-gray-300 mb-3">
                ...
                <span class="text-white font-medium">Out for Delivery Orders</span>
            </div>
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-semibold text-white mb-1">
                        🚚 Out for Delivery Orders ({{ $orders->count() }})
                    </h1>
                    <p class="text-sm text-gray-200">Manage orders currently out for delivery</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-3 text-center border border-white/20">
                    ...
                    <div class="text-xs text-gray-300">Out for Delivery</div>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-3 text-center border border-white/20">
                ...
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-3 text-center border border-white/20">
                    ...
                    <div class="text-xs text-gray-300">Today's Deliveries</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border border-white/20">
            ...
            <div class="bg-gradient-to-r from-[#1D293D] to-gray-800 px-4 py-3">
                <h2 class="text-lg font-medium text-white flex items-center gap-2">
                    <i class="fas fa-shipping-fast text-purple-300"></i>
                    Out for Delivery Orders Management
                </h2>
            </div>
            ...
            <div class="overflow-hidden">
                <div class="overflow-x-auto max-h-[600px]" style="scrollbar-width: thin; scrollbar-color: #8b5cf6 transparent;">
                    <table class="w-full table-fixed">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="group hover:bg-purple-50/30 transition-colors duration-200">
                                {{-- Order ID --}}
                                <td class="px-4 py-3">
                                    <div class="text-sm font-semibold text-purple-700 bg-purple-50 px-2 py-1 rounded-lg inline-block">
                                        #{{ $order->id }}
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="space-y-1">
                                        ...
                                        <div class="text-xs text-purple-600 bg-purple-50 px-2 py-1 rounded-full inline-block">
                                            {{ ucfirst(str_replace('_', ' ', $order->delivery_option ?? 'standard')) }} Delivery
                                        </div>
                                    </div>
                                </td>
                                ...
                                {{-- Status --}}
                                <td class="px-4 py-3">
                                    <span class="inline-flex items-center gap-1 bg-gradient-to-r from-purple-100 to-pink-100 text-purple-700 text-xs font-medium px-3 py-1 rounded-full border border-purple-200 animate-pulse">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div class="relative">
                                            <select onchange="updateOrderStatus('{{ $order->id }}', this.value)" 
                                                    class="bg-purple-500 hover:bg-purple-600 text-white px-3 py-1 rounded-lg text-sm border-0 cursor-pointer transition-colors duration-200">
                                        ...
                                        {{-- Print Delivery Slip --}}
                                        <button onclick="printDeliverySlip('{{ $order->id }}')" 
            ...
            <div class="text-center py-16">
                ...
                <h3 class="text-xl font-semibold text-gray-700 mb-2">No Orders Out for Delivery</h3>
                <p class="text-gray-500 mb-4">No orders are currently out for delivery.</p>
                <a href="{{ route('orders.processing') }}" class="inline-flex items-center gap-2 bg-purple-500 hover:bg-purple-600 text-white px-6 py-3 rounded-xl font-medium transition-colors duration-200">
...
function printDeliverySlip(orderId) {
    // Open delivery slip in new window
    window.open(`/orders/${orderId}/delivery-slip`, '_blank');
}
</script>
@endsection
