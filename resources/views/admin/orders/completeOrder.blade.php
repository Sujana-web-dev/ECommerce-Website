    ...
    <div class="p-4 space-y-6">
        ...
        <div class="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl p-4">
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route('orders.completed') }}" class="px-4 py-2 bg-gradient-to-r from-[#ec4642] to-red-600 text-white rounded-xl font-medium shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 flex items-center gap-2 text-sm">
        ...
        <div class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border border-white/20">
            ...
            <div class="overflow-hidden">
                <table class="w-full table-fixed text-xs">
                    <thead class="bg-gradient-to-r from-green-50 to-emerald-50">
                        <tr>
                            <th class="w-12 px-2 py-2 text-left text-xs font-medium text-gray-600 border-b border-gray-200">
                                ID
                            </th>
                            <th class="w-32 px-2 py-2 text-left text-xs font-medium text-gray-600 border-b border-gray-200">
                                Customer
                            </th>
                            <th class="w-36 px-2 py-2 text-left text-xs font-medium text-gray-600 border-b border-gray-200">
                                Products
                            </th>
                            <th class="w-20 px-2 py-2 text-left text-xs font-medium text-gray-600 border-b border-gray-200">
                                Total
                            </th>
                            <th class="w-20 px-2 py-2 text-left text-xs font-medium text-gray-600 border-b border-gray-200">
                                Status
                            </th>
                            <th class="w-24 px-2 py-2 text-left text-xs font-medium text-gray-600 border-b border-gray-200">
                                Date
                            </th>
                            <th class="w-24 px-2 py-2 text-left text-xs font-medium text-gray-600 border-b border-gray-200">
                                Actions
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="group hover:bg-green-50/30 transition-colors duration-200">
                                {{-- Order ID --}}
                                <td class="px-2 py-2">
                                    <div class="text-xs font-semibold text-green-700 bg-green-50 px-1.5 py-0.5 rounded inline-block">
                                        #{{ $order->id }}
                                    </div>
                                </td>
                                ...
                                <td class="px-2 py-2">
                                    <div class="space-y-0.5">
                                        <div class="text-xs font-medium text-gray-900 truncate" title="{{ $order->name }}">
                                            {{ Str::limit($order->name, 15) }}
                                        </div>
                                        <div class="text-xs text-gray-500 truncate">
                                            {{ Str::limit($order->user->name ?? 'Guest', 12) }}
                                        </div>
                                        <div class="text-xs text-gray-500 truncate">
                                            {{ Str::limit($order->email, 15) }}
                                        </div>
                                        <div class="text-xs text-gray-500 truncate">
                                            {{ Str::limit($order->phone, 12) }}
                                        </div>
                                    </div>
                                </td>
                                    ...
                                    <div class="space-y-0.5">
                                        @foreach($order->items->take(2) as $item)
                                            <div class="flex items-center gap-1">
                                                <span class="inline-block bg-green-100 text-green-800 text-xs px-1 py-0.5 rounded">
                                                    {{ $item->quantity }}x
                                                </span>
                                                <span class="text-xs text-gray-600 truncate">
                                                    {{ Str::limit($item->product->name, 18) }}
                                                </span>
                                            </div>
                                        @endforeach
                                ...
                                {{-- Status --}}
                                <td class="px-2 py-2">
                                    <span class="inline-flex items-center gap-1 bg-gradient-to-r from-green-100 to-emerald-100 text-green-700 text-xs font-medium px-2 py-0.5 rounded-full border border-green-200">
                                ...
                                <td class="px-2 py-2">
                                    <div class="flex items-center gap-1">
                                        <a href="{{ route('orders.show', $order->id) }}" 
                                           class="inline-flex items-center gap-1 bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-xs font-medium transition-colors duration-200">
                                            <i class="fas fa-eye"></i>
                                            View
                                        </a>
                                        
                                        <a href="{{ route('cart.invoice', $order->id) }}" 
                                           class="inline-flex items-center gap-1 bg-[#ec4642] hover:bg-red-600 text-white px-2 py-1 rounded text-xs font-medium transition-colors duration-200"
                                           target="_blank">
            ...
            <div class="text-center py-16">
                ...
                <a href="{{ route('orders.out_for_delivery') }}" class="inline-flex items-center gap-2 bg-[#ec4642] hover:bg-red-600 text-white px-6 py-3 rounded-xl font-medium transition-colors duration-200">
